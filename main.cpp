	...
	CXX=g++
	CPPFLAGS="--std=c++14 -Wall -Wno-sign-compare -O2 -g -DNDEBUG -fopenmp"
...
public:
	...
	virtual bool propagate(Output* output, int startIndex) const = 0;
...
public:
	...
	bool propagate(Output* output, int startIndex) const override;
...
bool OverlappingModel::propagate(Output* output, int startIndex) const
{
...
bool TileModel::propagate(Output* output, int startIndex) const
{
	boost::lockfree::queue<int> queue;
	std::atomic_int busy_counter;
	busy_counter = 0;

	for (int d = 0; d < 4; ++d) 
	{
		int x2 = startIndex % _width, y2 = startIndex / _width;
		int x1 = x2, y1 = y2;
...
Result observe(const Model& model, Output* output, RandomDouble& random_double, int& observedIndex)
{
	int argminx, argminy;
	...
	observedIndex = (model._width * argminy) + argminx;
...
Output create_output(const Model& model)
{
	...
	if (model._foundation != kInvalidIndex) {
		for (const auto x : irange(model._width)) {
			...
			while (model.propagate(&output, 0));
		}
	}
...
Result run(Output* output, const Model& model, size_t seed, size_t limit, jo_gif_t* gif_out)
{
	...
	for (size_t l = 0; l < limit || limit == 0; ++l) {
		...
		int observe_index = -1;
		Result result = observe(model, output, random_double, observe_index);
		...
		assert(observe_index >= 0);
		while (model.propagate(output, observe_index));
