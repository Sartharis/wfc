...
Result run(Output* output, const Model& model, size_t seed, size_t limit, jo_gif_t* gif_out, float& obs_avg, float& prop_avg)
{
        ...
        for (size_t l = 0; l < limit || limit == 0; ++l) {
                ...
                if (result != Result::kUnfinished) {
                        ...
                        obs_avg = avg_observation;
                        prop_avg = avg_propagation;
                        return result;
                }
...
void run_and_write(const Options& options, const std::string& name, const configuru::Config& config, const Model& model)
{
        ...
        float avg_observation = 0.0f;
        float avg_propagation = 0.0f;
        int runs = 0;
        ...
        for (const auto i : irange(screenshots)) {
                ...
                for (const auto attempt : irange(10)) {
                        ...
                        std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
                        float in_avg_observation = 0.0f;
                        float in_avg_propagation = 0.0f;
                        const auto result = run(&output, model, seed, limit, options.export_gif ? &gif : nullptr, in_avg_observation, in_avg_propagation);
                        avg_observation += in_avg_observation;
                        avg_propagation += in_avg_propagation;
                        runs++;
                        std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
                        std::chrono::duration<double> time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1);
        ...
        LOG_F(INFO, "%f to observe on average", avg_observation/(float)runs);
        LOG_F(INFO, "%f to propagate on average", avg_propagation/ (float)runs);
}
