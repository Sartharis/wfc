...
public:
	...
	virtual bool propagate_seq(Output* output) const= 0;
...
public:
	...
	bool propagate_seq(Output* output) const override;
...
bool OverlappingModel::propagate_seq(Output* output) const
{
...
bool TileModel::propagate_seq(Output* output) const
{
	bool did_change = false;
                           ...
                           int* argminx, int* argminy)
{
	...
	for (int x = 0; x < model._width; ++x) {
		for (int y = 0; y < model._height; ++y) {
			...
			// If a state is collapsed we expect there to be only 1 superimposed, not 0
